...
/**
 * Properties
 *  - K, V의 타입으로 String, String으로 단순화(고정) 시킨 컬렉션클래스
 *  - 설정정보 표현에 최적화된 타입
 *  - Generic X
 *
 * Properties 객체 -> 설정파일 출력
...
//			prop.load(fr);
	...
	private void test1() {
		Properties prop = new Properties();
		//요소 저장 = .setProperty(K, V)
		prop.setProperty("id", "honggd");
		prop.setProperty("password", "1234");
		prop.setProperty("birthday", "1999/09/09");
		
		//요소 가져오기 = .getProperty
		System.out.println(prop.getProperty("id")); //honggd
		System.out.println(prop.getProperty("password")); //1234
		System.out.println(prop.getProperty("birthday")); //1999/09/09
...
//			prop.store(fw, "user-info");
			prop.storeToXML(xmlfos, "user-info.xml"); //xml문서 출력용 메소드
		} catch (IOException e) {
			e.printStackTrace();
		} 
