...
//			request.setCharacterEncoding("utf-8");
			...
			// birthday와 hobby의 빈문자열로 제출되었을 때 분기처리
			String _birthday = request.getParameter("birthday"); //String -> sql Date
			Date birthday = null;
			if(_birthday != null && !"".equals(_birthday))
				birthday = Date.valueOf(_birthday);
			String[] _hobby = request.getParameterValues("hobby"); //String[] -> String
			String hobby = null;
			if(_hobby != null)
			...
			// 회원가입이 성공적으로 처리되었습니다. | 회원가입에 실패하였습니다. - 사용자 경고창 처리 
			request.getSession().setAttribute("msg", msg);
			response.sendRedirect(request.getContextPath() + "/");
		} catch (Exception e) {
			// 1. 로깅 및 관리팀 알림
			e.printStackTrace();
			// 2. 예외 던지기 - tomcat에 통보 - 예외페이지로 전환할 수 있게 한다.
			throw e;
		}
	}
